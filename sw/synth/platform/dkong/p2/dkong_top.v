...
//	`define SPARTAN_USE 
//--------------------------------------------------------------------------------
//-- include the VGA interface module --------------------------------------------
	`define VGA_USE
...
I_U1,I_D1,I_L1,I_R1,I_J1,
I_U2,I_D2,I_L2,I_R2,I_J2,
I_S1,I_S2,I_C1,
...
`ifdef SPARTAN_USE
BUFG CLK6M (.I(W_H_CNT[0]),  .O(WB_CLK_06144M));//  6.144MHz
BUFG CLK12M(.I(W_CLK_12288M),.O(WB_CLK_12288M));// 12.288MHz
BUFG ZCLK  (.I(W_H_CNT[1]),  .O(W_CPU_CLK));    //  3.072MHz
`endif
`ifdef CYCLONE_USE
...
assign O_ROM_AB   = ROM_A;
assign O_ROM_OEn  = 1'b0;
assign O_ROM_WEn  = 1'b1;
assign O_ROM_CSn  = 1'b0;
wire  [7:0]ROM_D = I_ROM_DB;
...
begin
  if(! W_RESETn)begin
    ...
    clk_d[1] <= clk_d[0];
    phase <= (~clk_d[1] & clk_d[0]) ? 0 : phase+1;
    case(phase)
       0: begin
         `ifdef SOUND_USE
      ...
      15: begin
          ROM_A       <= {3'h0,W_ROM_AB};
          R_AD <= R_AD==conf_cnt+1 ? R_AD : R_AD + 1 ;
          end
    default:;
   endcase 
  end
...
//----    DMA   ------------------------------------------
wire   [1:0]W_OBJ_A_offset = W_H_CNT[8]+1;
...
// input
.I_CLK_1(WB_CLK_06144M),
.I_CLK_2(WB_CLK_12288M),
.I_R(W_R),
.I_G(W_G),
.I_B(W_B),
.I_H_SYNC(~W_H_SYNCn),
...
assign O_SOUND_OUT_L = mix_sound;
assign O_SOUND_OUT_R = mix_sound;

`else
assign O_SOUND_OUT_L = 1'b1 ;
assign O_SOUND_OUT_R = 1'b1 ;
