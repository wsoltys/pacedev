...
volatile OS_EVENT* pe_event;
volatile int wait_for_me;
...
/*
*********************************************************************************************************
*                                      A SEMAPHORE WITH SEMAPORE PEND (Condition Variable)
*
* Description: This function aborts & readies any tasks currently waiting on a semaphore.  This function 
*              should be used to fault-abort the wait on the semaphore, rather than to normally signal
*              the semaphore via OSSemPost().
...
/*
 * Common sem task wait code.
 */
static OS_CPU_SR OSSemTaskWait (OS_CPU_SR cpu_sr, OS_EVENT *pevent, INT16U timeout, INT8U *perr)
{
    OSTCBCur->OSTCBStat     |= OS_STAT_SEM;           /* Resource not available, pend on semaphore     */
    OSTCBCur->OSTCBStatPend  = OS_STAT_PEND_OK;
    OSTCBCur->OSTCBDly       = timeout;               /* Store pend timeout in TCB                     */
...
void OSSemSemPend (OS_EVENT *pevent, INT16U timeout, OS_EVENT *plock, INT8U *perr)
{
    INT8U      my_cnt;
    INT8U      my_prio;
#if OS_CRITICAL_METHOD == 3                           /* Allocate storage for CPU status register      */
    ...
    if (plock == (OS_EVENT *)0) {                     /* Validate 'pevent'                             */
        *perr = OS_ERR_PEVENT_NULL;
        return;
    }
#endif
    ...
    if (plock->OSEventType != OS_EVENT_TYPE_SEM) {    /* Validate event block type                     */
        *perr = OS_ERR_EVENT_TYPE;
        return;
    }
    if (OSIntNesting > 0) {                           /* See if called from ISR ...                    */
    ...
    OS_ENTER_CRITICAL();
    my_prio = plock->OSEventCnt & OS_BSEM_KEEP_LOWER_8;
    my_cnt = (plock->OSEventCnt & OS_BSEM_KEEP_UPPER_8) >> 8;
    if ((my_cnt == 0) || (my_prio != OSTCBCur->OSTCBPrio)) {
        OS_EXIT_CRITICAL();
        *perr = OS_ERR_PEND_LOCKED;                   /* ... can't PEND unless locked                  */
        return;
    }
    plock->OSEventCnt = 0;                            /* unlock the mutex                              */
    if (plock->OSEventGrp != 0) {                     /* See if any task waiting for semaphore         */
                                                      /* Ready HPT waiting on event                    */
        (void)OS_EventTaskRdy(plock, (void *)0, OS_STAT_SEM, OS_STAT_PEND_OK);
    }
    /*
     * We only signal on the first sema (the condition variable).
     */
    if (pevent->OSEventCnt > 0) {                     /* If sem. is positive we have an error */
        *perr = OS_ERR_SEM_OVF;
    }
    else {
        /*
         ...
         */
        cpu_sr = OSSemTaskWait (cpu_sr, pevent, 0, perr);
        if (*perr == OS_ERR_PEND_ABORT) {
          /*
           * Obtain the mutex again.
           */
          volatile INT8U cnt;
          volatile INT8U prio;
          prio = plock->OSEventCnt & OS_BSEM_KEEP_LOWER_8;
          cnt = (plock->OSEventCnt & OS_BSEM_KEEP_UPPER_8) >> 8;
          while ((prio != OSTCBCur->OSTCBPrio) && (cnt != 0)) {
            cpu_sr = OSSemTaskWait (cpu_sr, plock, 0, perr);
            prio = plock->OSEventCnt & OS_BSEM_KEEP_LOWER_8;
            cnt = (plock->OSEventCnt & OS_BSEM_KEEP_UPPER_8) >> 8;
          }
          *perr = OS_ERR_NONE;
    ...
    plock->OSEventCnt = OSTCBCur->OSTCBPrio | (my_cnt << 8);
    OS_EXIT_CRITICAL();
    return;
}
