...
; - initialise GIME
	  ...
	  .endif				
				...
				lda			#0x00										; black
				sta			BRDR      							
				lda			#(VRAM_PG<<2)-1         ; screen at page $30
				sta			VOFFMSB
				lda			#0xFF
				sta			VOFFLSB   							
  .ifdef GFX_1BPP				
...
3$:     jsr     read_ascii_key          ; 6809 only
        ...
        jsr			joystick_active					; benign - for timing?
        dec     *timer
        bne     3$
        lda     #0
        jsr     display_char_pg2
...
1$:     lda     PIA1+DATAA
        ...
        bita    #(1<<7)                 ; joystick greater?
        beq     3$											; no, skip
        lda			#0x2e										; apple right threshold
2$:			sta			*joy_x
3$:
...
8$:			SEC															; flag active
				rts				
9$:			; delay for reading paddles
				; TBD
				CLC				
				rts
								
detect_paddles: ; $87A2
; we can't detect a Coco3 joystick
; - so always flag not present
