...
SW_Y = 850
        ...
        SWSDK_VCTR -20,-80,0
        lda     *snd
        jsr     SWSDK_RenderInt8Hex

        jsr     SWSDK_ButtonState               ; A (active high)
        ldb     *in01                           ; B=prev (active high)
        sta     *in01                           ; A=curr (prev=curr)
        eorb    *in01                           ; B=changed bits
        andb    *in01                           ; gone high
        bitb    #SWSDK_IN1_BUTTON2n             ; button 1 high?
        beq     1$
        inc     *snd
        bra     2$
1$:     bitb    #SWSDK_IN0_BUTTON1n
        beq     display_sdk_msg
2$:     lda     *snd
        jsr     SWSDK_WriteSoundCmd

display_sdk_msg:
        ; SDK message
        SWSDK_CNTR
        SWSDK_SCAL 2,0
        SWSDK_VCTR -256,-380,0
        SWSDK_COLOR SWSDK_GREEN,255
        ldx     #sdk_msg
        jsr     SWSDK_RenderString
        SWSDK_CNTR
        SWSDK_VCTR -300,-440,0
